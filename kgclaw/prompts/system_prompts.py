...
- **只抽取文本中明确出现的实体，不要推断或编造文本中没有的实体**

## 实体名称规范化规则
1. **精确匹配文本**: 实体名称应与文本中出现的原文字完全一致。不要添加或删除词汇。
2. **保留原文格式**: 文本中使用空格则保留空格，使用下划线则保留下划线。
3. **第三人称**: 所有实体名称和描述必须使用第三人称。
4. **避免代词**: 明确写出主体或客体名称；**禁止使用代词** 如 "本文"、"该公司"、"我"、"你"、"他/她"。
5. **全称优先**: 组织机构和人物使用全称。
6. **去除头衔**: 人名中去除职务头衔，但保留必要的区分信息。
7. **韩文/日文/中文**: 多语言实体保持原文格式，不要转换为拉丁字母或罗马化。
...
3. **防止幻觉**: 只抽取文本中明确出现的实体。不要从常识推断（如不要因为提到"微软"就推断一定有"比尔·盖茨"）。
4. 同一个实体多次出现只输出一次，以最完整的描述为准
5. 即使不确定也要抽取并标记低 confidence（<0.7），不要返回空列表
6. 能从文本中推断的属性一并提取（如日期、编号、职位等——但只在文本明确提及时）
7. **输出格式安全**: 格式模板中的示例不是源文本，不要从中提取实体
8. **置信度诚实**: confidence 反映实际确定性。明确提及的实体=0.9+，推断的=0.5-0.7
"""
...
- **只抽取文本中有明确证据支持的关系，不要推断文本中不存在的关联**

## 关系抽取优先级
1. **优先抽取高频关系**: 本体 Schema 中定义频率高的关系类型应优先检查
2. **位置/地点关系**: 文本中每对地点之间的关系通常是最重要的（如包含、位于、首都等）
3. **人物关系**: 人物与地点、组织之间的关系（如出生地、国籍、工作单位等）
4. **谨慎对待推断性关系**: 如职业、宗教信仰等，只在文本明确提及时才抽取
...
```json
{
  "relations": [
    {
      "subject": "主体实体名称（与实体列表中的名称一致）",
      "predicate": "关系类型（必须来自本体定义，精确匹配）",
      "object": "客体实体名称（与实体列表中的名称一致）",
...
## 规则
1. 只能抽取本体 Schema 中定义的关系类型，**关系名必须精确匹配**
...
7. **注意相似关系的区分**: 如 "country" vs "country of citizenship" vs "country of origin" 是不同含义的关系
8. **双向关系检查**: 某些关系有反向关系（如 A 包含 B ↔ B 位于 A），检查两个方向是否都存在
9. **避免过抽取**: 同一对实体之间优先使用最精确的关系类型，不要为同一事实重复抽取多个关系
"""
